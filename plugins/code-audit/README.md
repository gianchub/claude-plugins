...
4. **Systematic analysis** -- two-phase analysis:
...
5. **Large codebase strategy** -- partitions work across parallel subagents for codebases with 50+ files or 10,000+ lines of code, splitting along architectural boundaries for meaningful cross-file analysis within each partition
6. **Report generation** -- produces a structured `AUDIT-REPORT-YYYY-MM-DD.md` file with deduplicated findings ordered by severity, each including file location, category, description, impact assessment, and actionable recommendation. Includes a "Context & Intent" section documenting what design decisions were considered
