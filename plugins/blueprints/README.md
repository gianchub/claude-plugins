...
Transforms a task into a structured implementation plan through a collaborative dialogue. The skill explores the codebase to understand project structure and discover tooling (test runners, linters, type checkers, formatters), then presents the discovered tool chain for user confirmation before proceeding. Once tooling is confirmed, the skill asks clarifying questions to nail down scope and constraints, then proposes 2-3 implementation approaches with trade-offs before generating the plan. Plans adapt to complexity -- small changes get a single document, larger efforts get milestone folders with multiple files -- and to format: HTML by default (self-contained, with inline SVG dependency graphs for non-linear step dependencies and stable `data-status` attributes per step) or Markdown if requested. Format is silently inferred from the launching message; no proactive gate. A fast-path for narrowly scoped changes (≤2 steps) streamlines tool confirmation, approach selection, and plan review. Every step is written in prose (not code) and includes three phases: build instructions, adversarial review questions, and a verification checklist with concrete tool commands. After generation, an adversarial plan review subagent reads the plan cold and evaluates it for gaps, dependency issues, and architectural flaws before the user approves it for execution. Both the bulk codebase exploration and the plan review run in subagents that return distilled findings, so the planning conversation stays a lean coordinator rather than accumulating the contents of every file it surveyed.
...
Drives a blueprint plan through its build-review-verify cycles using dedicated subagents. The skill handles both `.md` and `.html` plan formats — format is detected by file extension, and progress mutations are applied to match (Markdown plans get `✅ ` heading prefixes and ticked `- [x]` checkboxes; HTML plans get `data-status="complete"`, status-badge text updates, `✅ ` h3 prefixes, and `checked` on every verification input). Steps are executed strictly serially, one at a time — every build, review, remediation, and verification runs inside its own subagent, so the coordinating conversation holds only their distilled summaries and its context stays lean across large, multi-milestone plans. A build subagent implements the step, a review subagent performs an adversarial code review reading files independently from disk, and a verification subagent runs actual tools and reports pass/fail. Ordinary defects do not stop the run: correctness, test, and integration findings that sit inside the step's scope are fixed by a remediation subagent, after which the review and verification both run again in full. The user is interrupted only for decisions that are genuinely theirs -- product or design ambiguity, a fix that would expand scope, anything destructive or reaching external state, a missing tool or credential, a checkpoint the plan asks for, or a step or subsystem that keeps failing (two remediation cycles per step, and a third cycle in one subsystem — once two different steps have contributed — escalates as an architectural decision rather than another patch). Escalations carry full error output and file references, and every auto-fix is reported per step and rolled up at each summary. In skill-managed mode, commits are never created for partial steps -- a step is committed only after its final state passes both review and verification. The skill supports two git modes: user-managed (pause after each step for manual commits) or skill-managed (automatic commits with adaptive pause cadence -- simple plans run straight through, complex multi-milestone plans pause at milestone boundaries by default with a full-auto option). Plan-progress mutations are written surgically (never full-file rewrites) for cross-session resumability.
...
3. Executes steps strictly serially, one at a time, dispatching a subagent for each phase — the coordinator keeps only each subagent's summary (never file contents or tool output), so its context stays lean:
...
5. Remediates in-scope findings automatically (a remediation subagent fixes the listed defects only, then review and verification re-run in full) and escalates the rest to the user with full error output and file references -- design ambiguity, scope expansion, destructive or external state, environment problems, plan checkpoints, and repeated failures in one step or subsystem. Nothing is silently skipped, no test or lint rule is ever weakened to make a check pass, and in skill-managed mode no commit is created until a step's final state passes both review and verification
...
7. Marks completed steps in the plan file with format-appropriate progress mutations (`✅ ` heading prefix + ticked checkboxes for Markdown plans; `data-status="complete"`, badge text update, `✅ ` h3 prefix, and `checked` on verification inputs for HTML plans) for cross-session resumability. Mutations are surgical — the rest of the plan file is left byte-identical
